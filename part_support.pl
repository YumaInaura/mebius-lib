...
my($hitflag,@line,$count_handler,$topics_only_flag);
...
our($concept,$enctrip);
	...
	# エラー
	if($my_use_device->{'bot_flag'}){ main::error("この環境からは拍手できません。"); }
...
my($thread) = Mebius::BBS::thread({ ReturnRef => 1 },$realmoto,$thread_number);
	...
	# 掲示板が投稿停止モードの場合
	if($main::stop_mode =~ /BBS/){ main::error("掲示板全体で、更新を停止中です。"); }

	# 拍手が禁止されている掲示板
	if($concept =~ /NOT-SUPPORT/){ main::error("この掲示板では拍手が出来ません。","","","Not-repair"); }

# アクセス制限
main::axscheck("Post-only");

	# 環境によってはエラー
	if($my_use_device->{'level'} < 2){ &error("この環境からは拍手が出来ません。","","","Not-repair"); }
	...
	# 連続拍手を制限
	#if($double_flag && !$alocal_mode){ &error("同じ記事に連続して拍手は出来ません。","","","Not-repair"); }
...
#$count += 1;
	...
	if(!$topics_only_flag){
		...
		my($craped) = Mebius::BBS::crap_file({ NewCrap => 1 , Renew => 1 , select_renew => \%select_renew , new_line => $new_line },$realmoto,$thread_number);
			if($craped->{'done_flag'} && !Mebius::AlocalJudge()){ main::error("この記事にはまだ拍手出来ません。","","","Not-repair"); }

		#open($count_handler,"<",$counter_file);
		#flock($count_handler,1);
		#my $top1 = <$count_handler>; chomp $top1;
		#my $top2 = <$count_handler>; chomp $top2;
		#	if($top2 eq ""){ $top2 = qq(<>); }
		#	while(<$count_handler>){
		#		chomp;
		#		push(@line,"$_\n");
		...
		# トップデータの追加
		#unshift(@line,"$top2\n");
		#unshift(@line,"$count<>$time<>$put_xips<>$put_cnumbers<>$put_res<>\n");
	}

	# ディレクトリを作成
	#if(!$topics_only_flag){
	#	Mebius::Mkdir(undef,"$main::bbs{'data_directory'}_crap_count_${realmoto}");
	#}

	# 拍手ファイルを書き込む
	#if(!$topics_only_flag){
	#	Mebius::Fileout("",$counter_file,@line);
	#}
...
#main::get_reshistory("KACCESS_ONE RENEW Crap-file New-crap My-file",undef,undef,$postdata_history);
	...
	if($my_use_device->{'level'} < 2){
		Mebius::Redirect("","$thread_number.html","301");
	}
	...
	# すぐ元記事に戻る
		...
		if($my_use_device->{'type'} eq "Mobile"){
			...
			&bbs_view_thread_mobile("Crap-done");
		}
		else{
			require "${init_directory}part_view.pl";
			&bbs_view_thread_desktop("Crap-done");
		}
	}
...
#main::get_reshistory("KACCESS_ONE RENEW Check-file New-check My-file",undef,undef,$postdata_history);
		...
		if($main::kflag){
			...
			require "${init_directory}part_view.pl";
			&bbs_view_thread_desktop("Thread-check-done");
		}


# ジャンプ先
#$main::jump_url = "http://$main::server_domain/_$realmoto/$thread_number.html";
#$main::jump_sec = 2;

# ヘッダ
#main::header("Body-print");

#print qq(<div class="supported">記事をチェックしました。更新があると「最近のレス」に表\示されます。削除する場合はマイページをご利用ください。 </div> );
...
($i_handle) = Mebius::Regist::HandleCheck(undef,$i_handle);
...
my $new_line = "1<>$i_handle<>$encid<>$enctrip<>$comment<>$pmfile<>$host<>$cnumber<>$put_age<>$time<>$date<>$new_resnum<><>\n";
	my($craped) = Mebius::BBS::crap_file({ NewComment => 1 , Renew => 1 , new_line => $new_line , select_renew => \%select_renew },$realmoto,$in{'no'});
		if(!$craped->{'done_flag'}){ &error("拍手した後でしかコメント登録は出来ません。","","","Not-repair"); }
		if($craped->{'comment_done_flag'}){ &error("連続して拍手コメントは送れません。"); }

	#($flag) = &support_check_double();
	#if(!$flag){ &error("拍手した後でしかコメント登録は出来ません。","","","Not-repair"); }
...
#if(time >= $lasttime + 1*24*60*60){ &error("前回の拍手から時間が経ちすぎています。","","","Not-repair"); }
